...
def loadTransferCostsWeb():
    ...
    transferCosts = [i.replace(' ', '_', 1).replace('_to_', ' ').replace(web, web_replacement).split() for i in
                     urllib.urlopen(transferCostURL)]
...
for n, costDict in enumerate(addList):
    ...
    values[headers.index('last_update')] = "TO_TIMESTAMP('" + values[
        headers.index('last_update')] + "', 'YYYY-MM-DD_HH24:MI:SS')"
