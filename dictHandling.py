...
def protoDict(queue, d, sourcestr='DB', keys=[]):
    ...
    return {param: dict([(key, d[queue][key]) for key in keys if key not in excl and key in d[queue].keys()]), over: {},
            source: dict([(key, sourcestr) for key in keys if key not in excl and key in d[queue].keys()]),
            enab: en_val}
    ...
    for queue in d:
        ...
        for key in out_d[c][d[queue][site]][d[queue][dbkey]][param]:
            if type(out_d[c][d[queue][site]][d[queue][dbkey]][param][key]) == list:
                try:
                    out_d[c][d[queue][site]][d[queue][dbkey]][param][key] = '|'.join(
                        out_d[c][d[queue][site]][d[queue][dbkey]][param][key])
                except TypeError:
                    if type(out_d[c][d[queue][site]][d[queue][dbkey]][param][key][0]) == dict:
                        try:
                            out_d[c][d[queue][site]][d[queue][dbkey]][param][key] = json.dumps(
                                out_d[c][d[queue][site]][d[queue][dbkey]][param][key])
                        except:
                            out_d[c][d[queue][site]][d[queue][dbkey]][param][key] = str(
                                out_d[c][d[queue][site]][d[queue][dbkey]][param][key])
            ...
            if key in booleanStringFields:
                out_d[c][d[queue][site]][d[queue][dbkey]][param][key] = booleanStrings[
                    out_d[c][d[queue][site]][d[queue][dbkey]][param][key]]
...
def BNL_ATLAS_1Deleter(d):
    ...
    for cloud in d:
        for site in [i for i in d[cloud] if i != All]:
            for queue in [i for i in d[cloud][site] if i != All]:
                if d[cloud][site][queue][param]['siteid'].startswith('BNL_ATLAS_1') or d[cloud][site][queue][param][
                    dbkey].startswith('BNL_ATLAS_1'):
