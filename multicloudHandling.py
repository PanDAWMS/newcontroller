...
class multicloudHandling:
    ...
    def getBestNT2DForT1(self, clouds):
        ...
        k = 0
        for i in rows:
            if stest != i['SITE_SOURCE'] and cloudss.find(i['CLOUD_SOURCE']) == -1 and clouds.find(
                    i['CLOUD_SOURCE']) != -1:
                k = 0
                stest = i['SITE_SOURCE']
    ...
    def updateMulticloud(self, clouds, matrix, matrix1):
        ...
        for i in matrix:
            ...
            if i['NICKNAME_DESTINATION'] == dest and float(
                    i['SONARLRGVAL']) >= multicloud_throughput_threshold_large and multicloud.count(',') < (
                        multicloud_number_of_sites_to_get - 1) and multicloud.find(
                i['CLOUD_SOURCE']) == -1 and clouds.find(
                i['CLOUD_SOURCE']) != -1:
                if len(multicloud) == 0:
        ...
        if multicloud.count(',') < (multicloud_number_of_sites_to_get - 1):
            for z in matrix1:
                if site_destination == z['SITE_DESTINATION'] and multicloud.count(',') < (
                            multicloud_number_of_sites_to_get - 1) and multicloud.find(
                    z['CLOUD_SOURCE']) == -1 and clouds.find(
                    z['CLOUD_SOURCE']) != -1:
                    multicloud += ',' + z['CLOUD_SOURCE']
        if len(multicloud) > 0 and multicloud != multicloud_old:
            self.InsertAndUpdate(dest, multicloud, multicloud_old)
    ...
    def InsertAndUpdate(self, nickname, multicloud, multicloud_old):
        ...
        sql = "INSERT INTO multicloud_history (site, multicloud, last_update) VALUES ('%s', '%s', SYSDATE) " % (
            nickname, multicloud_old)
        try:
